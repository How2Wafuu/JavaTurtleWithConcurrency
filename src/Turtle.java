...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    {
        new Thread(new Bob(-120,0,"blue",120,1)).start();
        new Thread(new Bob(-120, 0, "blue",120,-1)).start();
        new Thread(new Bob(-100,0,"red",100,1)).start();
        new Thread(new Bob(-100, 0, "red",100,-1)).start();
        new Thread(new Bob(-87,0,"blue",85,1)).start();
        new Thread(new Bob(-87, 0, "blue",85,-1)).start();
        new Thread(new Bob(-71,0,"red",60,1)).start();
        new Thread(new Bob(-71,0, "red",60,-1)).start();


        //At first, I wanted to draw an octopus, but it was too hard to make a curve line. ;-;
    }
